...
为了简单，在 lab2 的实验数据中只会出现 HashAgg, HashJoin, Sort, Selection, Projection, TableReader, TableScan, IndexReader, IndexScan, IndexLookup 这些算子，且不用考虑并发、Cache 等复杂的环境因素，只需要考虑算子的执行逻辑即可。
